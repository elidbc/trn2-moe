...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Dispatch tokens to experts and accumulate the results (XLA Safe Dense Masking)
        for expert_idx in range(self.num_experts):
            expert_layer = self.experts[expert_idx]
            
            # XLA requires static shapes, so we CANNOT use torch.where to filter tokens.
            # We must compute the expert on ALL tokens.
            current_hidden_states = expert_layer(hidden_states)
            
            # Get the specific routing weights for this expert
            # Multiply the global routing weights by the binary mask for this expert
            weight_mask = (routing_weights * expert_mask[expert_idx]).sum(dim=-1)
            
            # Apply the weights (tokens not routed to this expert will be multiplied by 0)
            current_hidden_states = current_hidden_states * weight_mask.unsqueeze(-1)
            
            # Accumulate into the final tensor
        ...
        # return experts as well so we can check for expert-level divergence
